...
### 3.1 provider 的最低实现要求

对于希望被当前 consumer 直接消费的 provider，最低实现要求主要集中在核心必需层：

- 与 AntiGEO entity schema 兼容的 entity 数据
- 与 override 兼容的本地覆盖语义
- 兼容当前 release 数据格式的 provider package
- `manifest`
- `entities.compact`
- topic / intent / risk / domain / name 索引

只要这些核心部分可用，provider 就已经可以进入当前最小消费链路。

### 3.2 optional provider extensions

以下内容当前属于 optional provider extensions，而不是所有 provider 的强制要求：
...
evidence 尤其应被理解为可选增强。它可以显著提升可解释性、可复核性与信任度，但 consumer 不应假设所有 provider 都会提供 evidence。
...
不同 provider 也可以采用不同透明度和治理深度。有些 provider 只发布核心 package；有些 provider 会额外发布 evidence、proposal 或更完整的治理记录。
...
### 3.1 Minimum Provider Requirements

For providers that want to be directly consumable by the current model, the minimum implementation is centered on the core required layer:

- entity data compatible with the AntiGEO entity schema
- local override compatibility at the consumer boundary
- a provider package compatible with the current release-data format
- `manifest`
- `entities.compact`
- topic / intent / risk / domain / name indexes

If those core parts are present, the provider is already usable within the current minimal flow.

### 3.2 Optional Provider Extensions

The following are currently optional provider extensions rather than mandatory requirements for every provider:
...
Evidence in particular should be understood as an optional enhancement. It can materially improve explainability, verifiability, and trust, but consumers should not assume that every provider will publish evidence.
...
Different providers may also choose different levels of transparency and governance depth. Some may publish only the core package, while others may additionally publish evidence, proposals, or richer governance records.
